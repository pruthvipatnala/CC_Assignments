...
url="http://3.85.102.74:80/api/v1/users"
app = Flask(__name__)
CORS(app)
...
def api_upload_act():
	...
	if request.method=='POST':
		...
		if not username in json.loads(requests.get("http://3.85.102.74/api/v1/users").text):
			print("invalid user")
			return jsonify({}),400
		try:
...
def health_checker():
	if request.method == "GET":
		...
		r = requests.post('http://3.85.102.74:80/api/v1/users',data = json.dumps(data))
		...
		print("API 2")
		r = requests.delete('http://3.85.102.74:80/api/v1/users/test_user').status_code
		stat_codes.append(r)
